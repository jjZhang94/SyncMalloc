...
__global__ void waitForHostAndContinue(struct MMUOnTransfer** pMMUOnTransfer, int *d_sizetmpAllocate, int *d_sizeTotalallocate, bool* d_isAllocate, int* d_freeBlock)
{
    ...
    // int* a = (int*)allocateMemoryPoolGPURandomlyStack(&s);
    int* a = (int*)HMCoAllocate(256, &s, pMMUOnTransfer, d_sizetmpAllocate, &store, d_sizeTotalallocate);
    // if(threadIdx.x == 0)
    // {
...
int main() 
{
    ...
    //     if(isAllocate[i] == 1)
    //     {
    //         if(abc[freeBlock[i]] == 1)
    //         {
    //             printf("step %d is error\n", i);
    //             return 1;
    //         }
    //         abc[freeBlock[i]] = 1;
    //     }else
    //     {
    //         if(abc[freeBlock[i]] == 0)
    //         {
    //             printf("step %d is error\n", i);
    //             return 1;
    //         }
    //         abc[freeBlock[i]] = 0;
    //     }
    //     //printf(" allocate or free i:%d %d %d\n", i, isAllocate[i], freeBlock[i]);
    // }

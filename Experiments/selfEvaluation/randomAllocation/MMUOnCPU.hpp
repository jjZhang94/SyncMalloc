...
#define MAX_BITMAP_MEM_SIZE (1024LL*1024LL*1024LL*16LL) //4GB
#define PAGE_SIZE (1024*4)
#define PAGE_NUMBER  (1024*1024*16)   // (MAX_BITMAP_MEM_SIZE/PAGE_SIZE) need 128KB
#define BITMAP_SIZE (1024*1024/8*16)   // (PAGE_NUMBER/8)
...
/* -- Version 1.3 */
#define PORTION_SIZE 256 // Each portion size in bytes
...
class BitmapManagement
{
    private:
        ...
        int isByteFree(unsigned int byteIndex);
...
typedef struct {
    void* key;
    unsigned int sizeAllocate;
} ThreadPointersSizeSecond;

typedef struct {
    ThreadPointersSizeSecond pairs[MAX_SECOND_ALLOCATE];
    int size;
} ThreadPointersSizeStore;
/*-- End Version 1.3.5*/
...
__device__ int deletePair(ThreadPointersSizeStore* store, void* key);

__device__ void* HMCoAllocate(size_t allocateSize, StackUsedPool *s, struct MMUOnTransfer** pMMUOnTransfer, int *d_sizetmpAllocate, ThreadPointersSizeStore* store, int *d_sizeTotalallocate);
/*-- End -- Version 1.3.5*/
#endif

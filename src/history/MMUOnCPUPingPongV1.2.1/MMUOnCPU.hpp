...
//     MMUOnTransfer(unsigned int threadNumber);
// }

/* -- End --  Define data structure for Transferring between CPU and GPU--*/

/*for total management*/
class ScalableAllocationManagement
{
    private:
        MemoryManagement *memoryManagement;
        MMUOnTransfer *pMMUOnTransfer;

    public:
        MMUOnTransfer *getPointerToMMUOnTransfer();
        int getTransferCounterForDoHOSTSync();

        ScalableAllocationManagement(unsigned int threadNumber);
        ~ScalableAllocationManagement();
};

// //a modification for launching kernel

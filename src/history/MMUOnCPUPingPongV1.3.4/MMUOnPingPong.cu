...
__global__ void waitForHostAndContinue(MemoryManagement* d_mm)
{
    //allocate call
    int tid =  threadIdx.x;
    int* a = (int*)d_mm->allocateMem(1024);
    // *a = tid + 20;
...
int main() 
{
    MemoryManagement* h_mm = new MemoryManagement();
    MemoryManagement* d_mm;
    cudaMalloc(&d_mm, sizeof(MemoryManagement));
    cudaMemcpy(&d_mm, &h_mm, sizeof(MemoryManagement), cudaMemcpyHostToDevice);
    ...
    waitForHostAndContinue<<<BLOCKNUMBER, BLOCKSIZE, 0, stream>>>(d_mm);

...
__global__ void waitForHostAndContinue(struct MMUOnTransfer* pMMUOnTransfer, int *d_sizetmpAllocate, int *d_sizeTotalallocate, bool* d_isAllocate, int* d_freeBlock)
{
    ...
    __syncthreads();
    int* c = (int*)allocateThr(512, pMMUOnTransfer, d_sizetmpAllocate, &store, d_sizeTotalallocate);
    if(threadIdx.x == 0)
    {
        printf("SDE %d, %p\n", blockIdx.x, c);
    }
...
int main() 
{
    ...
    thread_args args;
    int should_exit = 0;
    cudaError_t cudaStatus = cudaMallocManaged(&pMMUOnTransfer, sizeof(struct MMUOnTransfer)*BLOCKNUMBER);
    ...
    // Launch the kernel
    waitForHostAndContinue<<<BLOCKNUMBER, BLOCKSIZE, BLOCKSIZE*sizeof(int), stream>>>(pMMUOnTransfer, d_sizetmpAllocate, d_sizeTotalallocate, d_isAllocate, d_freeBlock);

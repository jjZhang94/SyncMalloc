...
{
    void* returnPtr = NULL;
    if(allocateSize > PORTION_SIZE)
    {
        returnPtr = allocateThr(allocateSize, pMMUOnTransfer, d_sizetmpAllocate, store, d_sizeTotalallocate);
    }else
    {
        returnPtr = (void *)allocateMemoryPoolGPURandomlyStack(s);
    }
}
/*-- End -- Version 1.3.5*/
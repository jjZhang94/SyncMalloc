...
void MemoryManagement::allocateMem(struct MMUOnTransfer* pMMUOnTransfer, int bid)
{
    ...
    if(pMMUOnTransfer -> sizeAllocate[bid]  * 8 > THRESHOLD_MEMSIZE)
    {
        pMMUOnTransfer -> addressAllocate[bid] = bitmapManagement->allocateMem(pMMUOnTransfer -> sizeAllocate[bid]);
    }else
    {
        pMMUOnTransfer -> addressAllocate[bid] = bitmapManagement->allocateMem(pMMUOnTransfer -> sizeAllocate[bid]);
    }
    printf("allocate %d address %p, siez %d\n", bid, pMMUOnTransfer -> addressAllocate[bid], pMMUOnTransfer -> sizeAllocate[bid]);
    sleep(2);
}
...
void MemoryManagement::freeMem(struct MMUOnTransfer* pMMUOnTransfer, int bid)
{
    ...
    printf("free %d address %p\n", bid, pMMUOnTransfer -> addressFree[bid]);
...
__device__ int sumAllocatieSize(struct MMUOnTransfer* pMMUOnTransfer, unsigned int sizeAllocate, int *d_sizeTotalallocate, ThreadPointersSizeStore* store)
{
    ...
    if (tid == 0)
    {
        pMMUOnTransfer->sizeAllocate[bid] = sdata[0];
...
__device__ void* allocateThr(size_t allocateSize, struct MMUOnTransfer* pMMUOnTransfer, int* d_sizetmpAllocate, ThreadPointersSizeStore* store, int *d_sizeTotalallocate)
{
    ...
    while (atomicAdd(&(pMMUOnTransfer -> syncFlag[bid]), 0) != 1) 
    {
    }
    __syncthreads();
    ...
    void* tmpStore =  (void*)((char*)pMMUOnTransfer -> addressAllocate[bid] + offset);
...
__device__ void freeThr(void* freeAddress, struct MMUOnTransfer* pMMUOnTransfer, ThreadPointersSizeStore* store, int *d_sizeTotalallocate, int* d_sizetmpAllocate)
{
    ...
    if(tid == 0)
    {
        ...
        {
            pMMUOnTransfer -> addressFree[bid] = freeAddress;
            pMMUOnTransfer -> syncFlag[bid] = 2;
            // printf("addFree %d, %d\n",sizeAllocate, index);

            while (atomicAdd(&(pMMUOnTransfer -> syncFlag[bid]), 0) == 2) 
            {
            }
        }
    }

...
class BitmapManagement
{
    private:
        ...
        unsigned char* d_bitmap;  // Bitmap to track memory blocks
        BlockIDSizeNode** d_buckets;  //used for storing the blockID and their size
        ...
        __device__ void setBitMapTrue(unsigned int blockIndex);  
        ...
        __device__ void setBitsMapTrue(unsigned int size, unsigned int startBlockIndex); 
        //This function clears a bit at a given index to indicate the block is free
        __device__ void clearBit(unsigned int blockIndex);
        //clears continuous bits at a given index to indicate the block is free and size of continuous bits.
        __device__ void clearBits(unsigned int size, unsigned int startBlockIndex);
        //This function checks if a bit at a given index is set
        __device__ bool isBitSet(unsigned int blockIndex);
        //Function to check if a bit at a given position in a byte is zero
        __device__ bool isZeroBit(unsigned char byte, unsigned int position);
        //
        //search suitable continuous pages
        __device__ unsigned int findPages(unsigned int pageSize);
        //search only one idle block in bitmaps
        __device__ unsigned int findPage();
        ...
        __device__ void insertHashTable(unsigned int key, unsigned int value);
        //Find of hash table, and return the size
        __device__ unsigned int findHashTable(unsigned int key);
        //Delete of hash table
        __device__ void deleteHashTable(unsigned int key);
    ...
    public:
        ...
        __device__ void* allocateMem(unsigned int sizeAllocate);
        //free memory with bitmaps
        __device__ void freeMem(void* addressIn);
};
...
class MemoryManagement
{
    private:
        ...
        BitmapManagement *d_bitmapManagement;
    ...
    public:
        ...
        __device__ void* allocateMem(unsigned int sizeIn);
        __device__ void freeMem(void *freeAddress);
};

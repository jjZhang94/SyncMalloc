...
BitmapManagement::BitmapManagement(void * startAddressIn)
{
    ...
    cudaMalloc(&d_bitmap, sizeof(char)*BITMAP_SIZE);
    cudaMemcpy(d_bitmap, bitmap, sizeof(char)*BITMAP_SIZE, cudaMemcpyHostToDevice);

    cudaMalloc(&d_buckets, sizeof(BlockIDSizeNode*)*HASHTABLE_BITMAP_BLOCKIDSIZE_SIZE);
    cudaMemcpy(d_buckets, buckets, sizeof(BlockIDSizeNode*)*HASHTABLE_BITMAP_BLOCKIDSIZE_SIZE, cudaMemcpyHostToDevice);
}
...
__device__ void BitmapManagement::setBitMapTrue(unsigned int blockIndex)
{
    unsigned int byteIndex = blockIndex / 8;
    unsigned int bitIndex = blockIndex % 8;
    unsigned char mask = 1 << bitIndex;
    d_bitmap[byteIndex] |= mask;
    return;
}

__device__ void BitmapManagement::setBitsMapTrue(unsigned int size, unsigned int startBlockIndex)
{
...
__device__ void BitmapManagement::clearBit(unsigned int blockIndex)
{
    unsigned int byteIndex = blockIndex / 8;
    unsigned int bitIndex = blockIndex % 8;
    unsigned char mask = ~(1 << bitIndex);
...
__device__ void BitmapManagement::clearBits(unsigned int size, unsigned int startBlockIndex)
{
    unsigned int endBlockIndex = startBlockIndex + size;
    unsigned int startByteIndex = startBlockIndex / 8;
    unsigned int endByteIndex = (endBlockIndex - 1) / 8;
...
__device__ bool BitmapManagement::isBitSet(unsigned int blockIndex)
{
    unsigned int byteIndex = blockIndex / 8;
    unsigned int bitIndex = blockIndex % 8;
    unsigned char mask = 1 << bitIndex;
    return (bool)(d_bitmap[byteIndex] & mask) != 0;    
}

__device__ bool BitmapManagement::isZeroBit(unsigned char byte, unsigned int position) 
{
    return (byte & (1 << position)) == 0;
}

__device__ unsigned int BitmapManagement::findPage()
{
...
__device__ void* BitmapManagement::allocateMem(unsigned int sizeAllocate)
{
...
__device__ void BitmapManagement::freeMem(void* addressIn)
{
...
__device__ unsigned int BitmapManagement::hash(unsigned int key)
{
...
__device__ void BitmapManagement::insertHashTable(unsigned int key, unsigned int value)
{
    ...
    newNode->next = d_buckets[idx];
    d_buckets[idx] = newNode;
}

__device__ unsigned int BitmapManagement::findHashTable(unsigned int key)
{
    unsigned int idx = hash(key);
    unsigned int returnValue;
    BlockIDSizeNode* current = d_buckets[idx];
    while (current != NULL) {
...
__device__ void BitmapManagement::deleteHashTable(unsigned int key)
{
    unsigned int idx = hash(key);
    BlockIDSizeNode *current = d_buckets[idx];
...
MemoryManagement::MemoryManagement()
{
    ...
    cudaMalloc(&d_bitmapManagement, sizeof(BitmapManagement));
    cudaMemcpy(d_bitmapManagement, bitmapManagement, sizeof(BitmapManagement), cudaMemcpyHostToDevice);
...
__device__ void* MemoryManagement::allocateMem(unsigned int sizeIn)
{
    return d_bitmapManagement->allocateMem(sizeIn);
...
__device__ void MemoryManagement::freeMem(void *addressIn)
{
    d_bitmapManagement->freeMem(addressIn);

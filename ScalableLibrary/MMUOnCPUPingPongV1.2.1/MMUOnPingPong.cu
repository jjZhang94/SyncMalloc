...
DTO
No block sychronizaitons

Version 1.2
dynamic allocation is running on CPU as an indepedent process
*/

//init meta data for management
ScalableAllocationManagement scalableAllocationManagement(THREADNUMBERTOTAL);
...
{
    // Launch the kernel
    waitForHostAndContinue<<<1, 2>>>(scalableAllocationManagement.getPointerToMMUOnTransfer());

    // The host code to be executed when the GPU sets the flag
    while (scalableAllocationManagement.isTransferCounterForDoHOSTSyncGreater0()) {

...
void MemoryManagement::allocateMem(struct MMUOnTransfer* pMMUOnTransfer)
{
    ...
    {
        if(pMMUOnTransfer -> sizeAllocate[i]  * 8 > THRESHOLD_MEMSIZE)
        {
            pMMUOnTransfer -> addressAllocate[i] = linkedListManagement->allocateMem(pMMUOnTransfer -> sizeAllocate[i]);
        }else
        {
            pMMUOnTransfer -> addressAllocate[i] = bitmapManagement->allocateMem(pMMUOnTransfer -> sizeAllocate[i]);
        }
    }
}
...
void MemoryManagement::freeMem(struct MMUOnTransfer* pMMUOnTransfer)
{
    ...
    for(int i = 0; i < BLOCKNUMBER*BLOCKSIZE; i++)
    {
        if((pMMUOnTransfer -> addressFree[i]) >= bitmapStartAddress && (pMMUOnTransfer -> addressFree[i]) < bitmapEndAddress)
        {
            bitmapManagement->freeMem(pMMUOnTransfer -> addressFree[i]);
        }else
        {
            linkedListManagement->freeMem(pMMUOnTransfer -> addressFree[i]);
        }
    }
}
...
/* -- End -- Implementation of data structure for Transferring between CPU and GPU--*/

/* -- Implementation of ScalableAllocationManagement for total management--*/
MMUOnTransfer* ScalableAllocationManagement::getPointerToMMUOnTransfer()
{
    return pMMUOnTransfer;
}

int ScalableAllocationManagement::getTransferCounterForDoHOSTSync()
{
    return pMMUOnTransfer -> counterForDoHOSTSync;
}

bool ScalableAllocationManagement::isTransferCounterForDoHOSTSyncGreater0()
{
    if(pMMUOnTransfer -> counterForDoHOSTSync >0)
    {
        return true;
    }else
    {
...
ScalableAllocationManagement::ScalableAllocationManagement(unsigned int threadNumber)
{
...
ScalableAllocationManagement::~ScalableAllocationManagement()
{
    delete memoryManagement;
    cudaFree(pMMUOnTransfer);
}
/* -- End -- Implementation of ScalableAllocationManagement for total management--*/

// // Implementation of new launching kernel library
// lanchKernel(unsigned int blockNumber, unsigned int blockSize)
// {

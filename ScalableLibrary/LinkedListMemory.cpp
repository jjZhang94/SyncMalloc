...
void* LinkedListManagement::allocateMem(unsigned int sizeAllocate)
{
    ...
    MemorySegmentNode* insertNodeLast = NULL;
    //find a suitable position to allocate and return the pointer to the last node.
    insertNodeLast = findSuitableMem(sizeAllocate);
    insertNode = insertNodeLast -> next;
    ...
    newMemorySegmentNode->length = sizeAllocate;
    newMemorySegmentNode->offsetAddress = insertNode -> offsetAddress;
    newMemorySegmentNode->next = insertNode;
    newMemorySegmentNode->isHole = false;

    //upadate original nodes
    insertNodeLast -> next = newMemorySegmentNode;
    insertNode -> length = insertNode -> length - sizeAllocate;
...
void BitmapManagement::setBitsMapTrue(size_t size, unsigned int startBlockIndex)
{
...
void BitmapManagement::clearBits(size_t size, unsigned int startBlockIndex)
{
    unsigned int endBlockIndex = startBlockIndex + size;
    unsigned int startByteIndex = startBlockIndex / 8;
    unsigned int endByteIndex = (endBlockIndex - 1) / 8;
...
unsigned int BitmapManagement::findPages(size_t pageSize)
{
...
void* BitmapManagement::allocateMem(unsigned int sizeAllocate)
{
    ...
    unsigned int pages = sizeAllocate / PAGE_SIZE;
    pages ++;
...
{
    for (int i = 0; i < HASHTABLE_BITMAP_BLOCKIDSIZE_SIZE; ++i) {
        BlockIDSizeNode* current = table->buckets[i];
        while (current != NULL) {
